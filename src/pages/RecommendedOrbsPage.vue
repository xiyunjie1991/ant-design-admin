<template>
      ...
      <script>
        import { FindRecommendedOrbs, FindOrbs,UpdateRecommendedOrbs  } from "@/api/orb";
        ...
        export default {
          ...
          methods: {
            ...
            update(){
                UpdateRecommendedOrbs(this.data.map(v=>{return v.id})).then(v=>{
                if(v.status === 0){
            ...
            async init() {
              this.loading = true;
              try {
                const r = await FindRecommendedOrbs({});
                if (r.status !== 0) {
