<template>
  <div class="root">
    <a-input-search placeholder="输入文章名称" style="width: 200px;margin-bottom:10px;" @search="onSearch" size="small"
      v-model="query" />
    <a-button-group>
      <a-select class="select" showSearch v-model="id" @search="remoteMethods" :showArrow="false" :filterOption="false"
        :defaultActiveFirstOption="true" :notFoundContent="null">
        <a-select-option v-for="item in articleList" :key="item.id">{{item.name}}</a-select-option>
      </a-select>
      ...
      <span slot="action" slot-scope="text, record, index">
        <a-dropdown>
          ...
          <a-menu slot="overlay">
            ...
            <a-menu-item>
              <a-popconfirm title="确定要删除猩球吗" @confirm="deleteOrb(record.id)" okText="Yes" cancelText="No">
  ...
  export default {
    ...
    methods: {
      ...
      async remoteMethods(query) {
        const r = await FindArticles(query, 1, 20)
        console.info("articleList", r.data)
      },
      async init() {
        this.articleList()
